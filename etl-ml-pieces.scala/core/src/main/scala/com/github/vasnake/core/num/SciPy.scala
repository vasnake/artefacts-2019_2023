...
object PCHIP {
  ...
  def findDerivatives(xs: Array[Double], ys: Array[Double]): Array[Double] = {
    ...
    if (ys.length == 2) Array(mk(0), mk(0))
      ...
      (1 until dk.length - 1).foreach(
        idx => if (!condition(idx - 1)) dk(idx) = 1.0 / wmean(idx - 1)
      )
  ...
  def fromDerivatives(xs: Array[Double], ys: Array[Double], dk: Array[Double]): Array[Array[Double]] = {
    require(xs.length == ys.length && ys.length == dk.length, "xi and yi and dk need to have the same length")
    require(xs.indices.forall(idx => idx == 0 || (xs(idx) - xs(idx - 1)) > 0), "x coordinates are not in increasing order")
    ...
    (0 until m).foreach(idx => {
      val b: Array[Double] = _constructFromDerivatives(xs(idx), xs(idx + 1), ys(idx), ys(idx + 1), dk(idx), dk(idx + 1))
      c(idx) = b
    })
  ...
  def _constructFromDerivatives(xa: Double, xb: Double, ya: Double, yb: Double, da: Double, db: Double): Array[Double] = {
  ...
  def _edgeCase(h0: Double, h1: Double, m0: Double, m1: Double): Double = {

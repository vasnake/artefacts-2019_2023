 ...
 *
  ...
  */
case class FeaturesGroup(name: String, index: Array[String], isArray: Boolean = false) {
  override def toString: String = s"""FeaturesGroup("${name}", Array(${index.mkString(", ")}), isArray = ${isArray})"""
}
...
case class GroupedFeatures(groups: Seq[FeaturesGroup]) {
  ...
  def groupIndices(name: String): Option[Array[String]] = GroupedFeatures.groupNumber(groups, name).map(groups(_).index)
...
object GroupedFeatures {
  ...
  (features: Array[T], indices: Array[Int])
  : Array[T] = {
  ...
  def computeFeaturesIndices(mergedGfs: GroupedFeatures, gfs: GroupedFeatures): Array[Int] = {
    val res = (gfs.groups foldLeft Vector.empty[Int])((vector, group) => {
      ...
      val groupNumber = mergedGfs.groupNumber(group.name).getOrElse(
        throw new IndexOutOfBoundsException(s"can't find group '${group.name}' in merged groups '${mergedGfs.groups.map(_.name)}'")
      )
      ...
      val groupIndices = group.index.map(name =>
        name2index.getOrElse(name,
          throw new IndexOutOfBoundsException(s"can't find feature name '${name}' in group '${mergedGroup}'")
        ) + prefixSize
      )
  ...
  private def updateGroupedFeatures(groupedFeatures: Map[String, Array[String]], fg: FeaturesGroup): Map[String, Array[String]] = {
    if (groupedFeatures.isDefinedAt(fg.name)) groupedFeatures.updated(
      fg.name,
  ...
  private def merge(groupedFeatures: Map[String, Array[String]], itm: GroupedFeatures): Map[String, Array[String]] = {

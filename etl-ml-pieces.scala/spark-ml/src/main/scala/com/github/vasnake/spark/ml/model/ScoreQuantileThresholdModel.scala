...
import com.github.vasnake.`ml-core`.models.{ScoreQuantileThreshold, ScoreQuantileThresholdConfig}
...
class ScoreQuantileThresholdModel(override val uid: String, val groupsConfig: List[(String, ScoreQuantileThresholdConfig)]) extends
  Model[ScoreQuantileThresholdModel] with
  MLWritable with
  ...
  ParamsServices
{
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    logInfo(s"Transform dataframe, ${groupsConfig.length} models for groups: (${groupsConfig.map(_._1).mkString(", ")}), ...")
    ...
    val cfg: Broadcast[ScoreQuantileThresholdModelConfig] = df.sparkSession.sparkContext.broadcast(
      ScoreQuantileThresholdModelConfig(
        models = groupsConfig.map { case (group, cfg) => (group, ScoreQuantileThreshold(cfg)) }.toMap,
...
object ScoreQuantileThresholdModel {
  ...
  def transform(df: DataFrame, config: Broadcast[ScoreQuantileThresholdModelConfig]): DataFrame = {
    ...
    df.mapPartitions(rows => {
      ...
      @inline def makeResultRow(row: Row, cls: => Any, rank: => Any): Row = {
      ...
      rows.map(row => {
        if (row.isNullAt(cfg.inputColIndex)) makeResultRow(row, null, null) // invalid input
          ...
          cfg.models.get(row.getString(cfg.groupColIndex)).map(model => {

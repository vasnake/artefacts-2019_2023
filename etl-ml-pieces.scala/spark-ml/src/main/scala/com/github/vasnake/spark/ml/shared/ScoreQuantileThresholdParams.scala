...
/**
  ...
  */
trait ScoreQuantileThresholdParams extends
  StratifiedModelParams with
  HasRankCol with
  HasPriorValues with
  HasLabels with
  SamplingModelParams with
  HasCacheSettings
{
  ...
  override def validateAndTransformSchema(schema: StructType): StructType = {
    ...
    val withLabel = if (isDefinedOutputCol) schema.add(StructField(getOutputCol, DataTypes.StringType)) else schema

    if (isDefinedRankCol) withLabel.add(StructField(getRankCol, DataTypes.DoubleType)) else withLabel
  }

...
/** Calculates score label (class index) according to distribution given in `prior` parameter.
  ...
  */
class ScoreQuantileThresholdEstimator(override val uid: String)
    extends Estimator[ScoreQuantileThresholdModel]
  ...
  override def fit(dataset: Dataset[_]): ScoreQuantileThresholdModel = {
    ...
    val configs: List[(String, models.ScoreQuantileThresholdConfig)] = {
      ...
      val group_model: Array[(String, String, Option[models.ScoreQuantileThresholdConfig])] =
        ScoreQuantileThresholdEstimator
...
object ScoreQuantileThresholdEstimator {
  def distributedFit(group_score: DataFrame, prior: Array[Double])
    : Dataset[(String, String, Option[models.ScoreQuantileThresholdConfig])] = {
  ...
  def fit(scoreRaw: Iterator[Double], prior: Array[Double]): FitResult = Try {
    ...
    val msg: String =
      if (xs.length < prior.length)
        "Fitting quantiles to scores vector xs, xs.length < classes.length"
      else ""

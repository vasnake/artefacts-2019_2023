...
import org.apache.spark.sql.{ functions => sqlf, _ }

/** NEPriorClassProba model (transformer), fitted in NEPriorClassProbaEstimator
  */
class NEPriorClassProbaModel(override val uid: String, val groupsConfig: List[(String, MLV)])
    extends Model[NEPriorClassProbaModel]
       with MLWritable
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    logInfo(
      s"Transform dataframe, ${groupsConfig.length} aligners for groups: (${groupsConfig.map(_._1).mkString(", ")}), ..."
    )
    ...
    val cfg: Broadcast[NEPriorClassProbaConfig] = df
      .sparkSession
      .sparkContext
      .broadcast(
...
object NEPriorClassProbaModel {
  ...
  def json(x: List[(String, MLV)]): String = jsonify(x.map {
    case (name, vec) => s"group: $name; vec: ${vec.toString}"
  }.toArray)
}

...
/**
  ...
  */
trait ScoreEqualizerParams extends StratifiedModelParams with HasNumClasses with SamplingModelParams with HasRandomValue with HasCacheSettings
{

  final val numBins: IntParam = new IntParam(this, "numBins", "Fit: number of bins for spline interpolator", isValid = ParamValidators.gt(0))
  ...
  final val epsValue: DoubleParam = new DoubleParam(this, "epsValue", "Epsilon parameter for unit stretching")
  def getEpsValue: Double = $(epsValue)
  def setEpsValue(value: Double): this.type = set(epsValue, value)

  final val minInputSize: IntParam = new IntParam(this, "minInputSize", "Fit: minimum size of train vector", isValid = ParamValidators.gt(0))

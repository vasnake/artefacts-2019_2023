...
class NEPriorClassProbaTest extends AnyFlatSpec with should.Matchers with LocalSpark with DataFrameHelpers {
  ...
  it should "pass smoke test" in {
    ...
    val output = model.setInputCol("probs").setOutputCol("probs_aligned").setGroupColumns(Array.empty[String])
      .transform(input)
      .cache()
  ...
  it should "pass reference test B" in {
    ...
    val output = model.setOutputCol("probs_aligned")
      .transform(input)
      .cache()
  ...
  it should "pass reference test with refdata.json" in {
    ...
    val input: DataFrame = referenceData.toDataFrame(spark) // (uid, xs, probs)
  ...
  it should "consider non-existent model as invalid data" in {
    ...
    val validInput = inputDF.selectExpr("uid", "probs", "expected_probs_aligned").where("part = 'J'")
  ...
  it should "transform to null with unfitted model" in {
    ...
    val validInput = inputDF.selectExpr("uid", "probs", "expected_probs_aligned").where("part = 'J' and uid != 'h'")
  ...
  it should "fit/apply different models with stratification" in {
    ...
    val input = inputDF.selectExpr("uid", "part", "probs", "expected_probs_aligned").where("part in ('K', 'L')")
  ...
  it should "check parameters on validation stage" in {
    ...
    model.setInputCol("probs").setOutputCol("probs_aligned").setGroupColumns(Array.empty[String])
      .transform(input).show()

    // input col not exists

    assert(intercept[sql.AnalysisException] {
      model.setInputCol("_probs")
        .transform(input).show()
    }.getMessage.contains("""cannot resolve '`_probs`'"""))

    assert(intercept[sql.AnalysisException] {
      model.setInputCol("probs")
    ...
    // output col exists
    assert(intercept[IllegalArgumentException] {
      model.setInputCol("probs").setGroupColumns(Seq.empty)
    ...
    // prior contains x <= 0
    assert(intercept[IllegalArgumentException] {
      estimator.setInputCol("probs").setGroupColumns(Seq.empty)
    ...
    // n_classes < 2
    assert(intercept[IllegalArgumentException] {
      estimator.setPriorValues(Seq(1))
        .fit(input)
  ...
  it should "filter DF using sql array functions" in {
    ...
    // array.sum = 0
    show(input.selectExpr("uid",
      "aggregate(probs, cast(0 as double), (acc, x) -> acc + cast(x as double)) as sum"
    ), "aggregate")
    check(expr = "aggregate(probs, cast(0 as double), (acc, x) -> acc + cast(x as double)) = 0", expected = "b1,g,h".split(','))

    // one valid record
    check(expr =
      "probs is not null and " +
        "size(probs) = 3 and " +
    ...
    def check(expr: String, expected: Iterable[String]): Assertion =
      input.where(expr).selectExpr("uid").as[String]
        .collect().toList should contain theSameElementsAs expected.toList
  ...
  it should "not fail if data col name is x" in {
    ...
    val estimator = new NEPriorClassProbaEstimator()
      .setInputCol("x").setPriorValues(Seq(1.0, 1.0))

    val model = estimator.fit(input)

    val output = model.setOutputCol("y")
      .transform(input)
      .cache()
  ...
  private def assertVectors(actual: Iterable[Double], expected: Iterable[Double], accuracy: Int = 7): Assertion = {
    val a = actual.toList
  ...
  private def assertResult(accuracy: Int)(actual: DataFrame, expected: DataFrame): Assertion = {
    ...
    compareArrayDoubleColumns(actual, expected, "probs_aligned", "expected_probs_aligned", accuracy = accuracy)
...
object NEPriorClassProbaTest {
  ...
  case class InputRow(uid: String, part: String, probs: Option[Array[Option[Double]]], expected_probs_aligned: Option[Array[Option[Double]]])
  object InputRow {

    def apply(uid: String, part: String, probs: String, expected_probs_aligned: String): InputRow =
      InputRow(
        uid,
        part,
  ...
  case class ReferenceData(
    ...
    require(xs.length == probs.length && xs.head.length == prior.length, "In/out data shapes must be equal. [1]")
    require(xs.indices.forall(i => xs(i).length == probs(i).length), "In/out data shapes must be equal. [2]")

    def toDataFrame(implicit spark: SparkSession): DataFrame =
      spark.createDataFrame(xs.indices.map(i =>
        ReferenceRow(
          uid = s"uid_${i}",
          xs = xs(i),
          probs = probs(i)
  ...
  case class ReferenceRow(uid: String, xs: Array[Double], probs: Array[Double])

...
object PrettyPrint {
  ...
  def jsonify[A](x: A)(implicit cap: Jsonifier[A]): String = cap.doJsonification(x)
  ...
  object implicits {
    ...
    implicit val jlist: Jsonifier[List[_]] = new Jsonifier[List[_]] {
      override def doJsonification(xs: List[_]): String = xs.map {
    ...
    implicit val jmap: Jsonifier[Map[_, _]] = new Jsonifier[Map[_, _]] {
      override def doJsonification(xs: Map[_, _]): String = xs.map {
        case x: AnyRef => jtb.writeObject(x)
        case v => s"$v"
      }.mkString("[", ",", "]")
    }
    ...
    implicit val jarray: Jsonifier[Array[_]] = new Jsonifier[Array[_]] {
      override def doJsonification(xs: Array[_]): String = xs.map {
        case x: AnyRef => jtb.writeObject(x)
        case v => s"$v"
      }.mkString("[", ",", "]")
    }

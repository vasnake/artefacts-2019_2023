...
import com.github.vasnake.text.parser.JoinExpressionParser.{Tree, Node}
...
final case class TreeJoin[T](tree: Tree) extends JoinExpressionEvaluator[T] {
  override def eval[B](joinOp: (B, B, String) => B)(implicit dir: String => T, ev: T => B): B = {
    tree match {
      case Tree(left: Node, right: Node, jo) => joinOp(dir(left.name), dir(right.name), jo)
      case Tree(left: Node, right: Tree, jo) => joinOp(dir(left.name), TreeJoin[T](right).eval(joinOp)(dir, ev), jo)
      case Tree(left: Tree, right: Node, jo) => joinOp(TreeJoin[T](left).eval(joinOp)(dir, ev), dir(right.name), jo)
      case Tree(left: Tree, right: Tree, jo) => joinOp(TreeJoin[T](left).eval(joinOp)(dir, ev), TreeJoin[T](right).eval(joinOp)(dir, ev), jo)
    }
  }
}

...
//  import VectorizationTest._
  ...
  it should "build row decoder from df.schema and grouped features index" in {
    ...
    val gfIndex = GroupedFeatures(Seq(
  ...
  it should "extract features for multiple models 2" in {
    ...
    check(GroupedFeatures.sliceFeaturesVector(allFeatures, GroupedFeatures.computeFeaturesIndices(mergedGfs, gfs1)), expected1)
    check(GroupedFeatures.sliceFeaturesVector(allFeatures, GroupedFeatures.computeFeaturesIndices(mergedGfs, gfs2)), expected2)
  }

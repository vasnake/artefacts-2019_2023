...
object StratifiedSampler extends GroupingColumnsServices with CustomLogging {
  ...
  def getGroupScoreSample(dataset: Dataset[_], cfg: SelectGroupScoreSampleConfig): DataFrame = {
    ...
    val group_score_sample: DataFrame = {
      ...
      val group_count: Array[(String, Long)] = group_score
        .groupBy("group")
        .count()
        .as[(String, Long)]
        .collect()
  ...
  private def sample(
    ...
    val fractions: Map[String, Double] = group_size.map {
      case (group, count) =>
...
trait GroupingColumnsServices {
  val TEMP_GROUP_COLUMN: String = "_temp_concatenated_grouping_columns_" // TODO: name should be calculated dynamically, based on DF schema
  ...
  def dropTempGroupingColumn(df: DataFrame, groupColName: String = TEMP_GROUP_COLUMN): DataFrame =
    df.drop(groupColName)

  def addGroupToGroupingKey(key: String, group: String): String =
    s"${key}/${if (group.isEmpty) "-" else group}"
}

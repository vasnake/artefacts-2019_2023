...
object FeaturesRowDecoder {
  def apply(schema: StructType, fgroupsColl: GroupedFeatures): FeaturesRowDecoder = {
    val decoders: Seq[CollectionColumnDecoder] =
      fgroupsColl.groups.map(group => CollectionColumnDecoder(group.name, schema, group.index))
...
object CollectionColumnDecoder {
  private val mapFieldDefaults =
    FieldDefaults(nullField = 0, noFeatureKey = 0, nullFeature = Double.NaN)
  private val arrayFieldDefaults =
    FieldDefaults(nullField = Double.NaN, noFeatureKey = Double.NaN, nullFeature = Double.NaN)
  ...
  def apply(
    ...
    schema(fieldIndex).dataType match {
      ...
      case ArrayType(DataTypes.FloatType, _) =>
        ArrayColumnDecoder[Float](
          fieldIndex,
          featuresIndices.map(_.toInt),
  ...
  override protected def writeFeatures(
    ...
    featuresIndices.foreach { fi =>
      vec(idx) = toDouble {
        ...
        else if (field(fi) == null) featureValueIfValueIsNull

...
(
  ...
  def apply(rows: Seq[Row]): Any = {
    ...
    val result: Option[mutable.WrappedArray[Any]] = rows.find(row => !row.isNullAt(colIdx))
      .map(row => row.getAs[mutable.WrappedArray[Any]](colIdx).clone())

    val numRows = rows.length

    result.foreach(res => res.indices.foreach(idx => {
...
(
  ...
  def copy(): SetOfNamedPipelines = SetOfNamedPipelines(default.copy(), features.map(kv => kv._1 -> kv._2.copy()))
}

...
class PrimitiveLongTest extends AnyFlatSpec with should.Matchers with LocalSpark with Checks {
  ...
  lazy val inputDF: DataFrame = cache(
    createInputDF(spark)
      ...
      .selectExpr("part", "uid", "cast(feature as long) as feature")
      .repartition(4)
  )

  val resultColumnType: String = "LongType"

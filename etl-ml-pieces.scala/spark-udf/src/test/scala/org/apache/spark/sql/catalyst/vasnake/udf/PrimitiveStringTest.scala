...
class PrimitiveStringTest extends AnyFlatSpec with should.Matchers with LocalSpark with Checks {
  ...
  lazy val inputDF: DataFrame = cache(
    createInputDF(spark)
      .where("feature is null or feature not in (double('NaN'), double('-Infinity'), double('Infinity'))")
  ...
  it should "pass smoke test, most_freq native API" in {
    ...
    show(
      input.groupBy("part").agg(sql.Column(
  ...
  it should "pass smoke test, most_freq DataFrame API" in {
    ...
    show(output, message = "generic imperative most_freq output, DataFrame API shortcut 2", force = true)
  }
  ...
  it should "pass smoke test, most_freq SQL API" in {
    ...
    val prefer = "1"
    show(
      spark.sql(s"select part, gmf(feature, ${index}, ${threshold}, ${prefer}) from features group by part"),
  ...
  it should "select value by random index if there is a tie, part M, most_freq" in {
    ...
    while (set.size < 2 && iterNo < 1000) {
      iterNo += 1
      set += input.groupBy("part").agg(generic_most_freq("feature") as "mfq").head().getAs[String]("mfq")
    }

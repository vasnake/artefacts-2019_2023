...
class PrimitiveShortTest extends AnyFlatSpec with should.Matchers with LocalSpark with Checks {
  ...
  lazy val inputDF: DataFrame = cache(
    createInputDF(spark)
      ...
      .selectExpr("part", "uid", "cast(feature as short) as feature")
      .repartition(4)
  )

  val resultColumnType: String = "ShortType"

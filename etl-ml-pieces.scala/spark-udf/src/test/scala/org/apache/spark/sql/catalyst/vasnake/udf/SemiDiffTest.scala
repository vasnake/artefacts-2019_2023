...
class SemiDiffTest extends AnyFlatSpec with DataFrameHelpers with LocalSpark {
  ...
  override protected def beforeAll(): Unit = {
    ...
    functions.registerAs(funcName = "generic_semidiff", targetName = "generic_semidiff", spark, overrideIfExists = true)
  }
  ...
  private def checkType(elementType: String, sparkElementType: DataType) = {
    ...
    val actual = spark.sql(s"select generic_semidiff(${params}) as ${colName}").persist(StorageLevel.MEMORY_ONLY)
  ...
  it should "produce reference values" in {
    ...
    val actual = data
      .selectExpr("upper(uid) as uid", "semidiff(cast(va as array<double>), cast(vb as array<double>)) as semidiff", "expected")
    ...
    assert(actual.schema("semidiff").toString() === "StructField(semidiff,ArrayType(FloatType,true),true)")

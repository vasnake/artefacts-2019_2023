...
object Fixtures {
  ...
  object InputRow {
    ...
    def apply(part: String, uid: String, feature: String, feature_list: String, feature_map: String): _Row =
      _Row(
        part, uid,
  ...
  case class MapTransformer(df: DataFrame, filter: String) {
    ...
    def time_int: DataFrame = map[String, Int, java.sql.Timestamp, Int](
      k => Try(java.sql.Timestamp.valueOf(k)).getOrElse(java.sql.Timestamp.valueOf("2021-05-12 12:34:55")),
      v => v
    )
    ...
    def time_decimal: DataFrame = map[String, sql.types.Decimal, java.sql.Timestamp, sql.types.Decimal](
      k => Try(java.sql.Timestamp.valueOf(k)).getOrElse(java.sql.Timestamp.valueOf("2021-05-12 12:34:55")),
      v => v.toPrecision(4, 3)
    ).selectExpr("part", "uid", "cast(feature as map<timestamp, decimal(4,3)>) as feature")

    def map[IK, IV, OK, OV](keyTransform: IK => OK, valueTransform: IV => OV)(implicit
                                                                              ie: Encoder[(String, String, Map[IK, Option[IV]])],
                                                                              oe: Encoder[(String, String, Map[OK, Option[OV]])]
    ): DataFrame = {

      df.where(filter).as[(String, String, Map[IK, Option[IV]])].map {
        case (part: String, uid: String, feature: Map[IK, Option[IV]]) =>
          val f: Map[OK, Option[OV]] = feature.map{ case (k, v) => (keyTransform(k), v.map(valueTransform)) }
          (part, uid, f)

...
class NumericAccumulator extends OpenHashMap[NumericAccumulator.K, NumericAccumulator.V] {
  ...
  def containerIsNull(isnull: Boolean): Unit = update(null.asInstanceOf[K], if (isnull) 1.0 else -1.0)
...
object NumericAccumulator {
  ...
  def valConverter(targetType: DataType): (Any => Any) = targetType match {
    ...
    case DecimalType.Fixed(precision, scale) => { v: Any => if (v == null) null else Decimal(BigDecimal(v.asInstanceOf[V]), precision, scale) }
    case _ => throw new IllegalArgumentException(s"Accumulator: unknown value type: `${targetType}`")
  }

  def rowToBufferValueConverter(srcType: DataType): Any => V = srcType match {
    case DecimalType.Fixed(_, _) => { v: Any => if (v == null) null else v.asInstanceOf[Decimal].toDouble }

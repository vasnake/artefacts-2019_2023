    ...
    with Logging
{
  ...
  override def eval(buffer: NumericAccumulator): Any = {
    ...
    if (isArray) {
      ...
      if (buffer.isEmpty) null else generateOutputPrimitive(buffer)
    }
  }
  ...
  @inline protected def updateBuffer(buffer: NumericAccumulator, key: NumericAccumulator.K, value: NumericAccumulator.V): Unit =
  ...
  @transient protected lazy val codec: ColumnCodec = child.dataType match {
    ...
    case _ => throw new IllegalArgumentException(s"GenericAggregate: unknown expression data type: ${child.dataType}")
  }

...
import org.apache.spark.sql.catalyst.vasnake.udf.accum.{Accumulator, NumericAccumulatorWithCount => AccImpl}
                      ...
                      ) extends GenericCountItems
{
  ...
  def this(child: Expression, indexExpression: Expression, thresholdExpression: Expression, preferExpression: Expression) =
  ...
  @inline def evalItem(a: Accumulator): AccImpl.K = {
    ...
    def filterSortSelect(items: Array[(AccImpl.K, AccImpl.VC)]): AccImpl.K = {
      ...
      val filtered = threshold.map(td => {
        val total: Double = items.map(x => x._2.toLong).sum.toDouble
        assume(total > 0 || items.isEmpty, "Require sum of counts > 0 for non empty list of items")
      ...
      if (filtered.isEmpty) null
        ...
        val sorted = filtered.sortWith { case (a, b) => {
    ...
    preferredResult.getOrElse(filterSortSelect(
    ...
    with Logging
{
  ...
  @transient protected lazy val codec: ColumnCodec = {
    ...
    val res = child.dataType match {
      ...
      case _ => throw new IllegalArgumentException(s"GenericCountItems: unknown expression data type: ${child.dataType}")
    }

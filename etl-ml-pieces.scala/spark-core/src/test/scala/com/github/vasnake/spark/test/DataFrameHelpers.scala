...
trait DataFrameHelpers extends should.Matchers {
  ...
  def show(df: DataFrame, message: String = "", nrows: Int = 100, force: Boolean = false): Unit =
    if (debugMode || force) {
  ...
  def compareSchemas(actual: StructType, expected: StructType, checkNull: Boolean = false): Unit = {
    assert(actual.length === expected.length)

    actual.fields.sortBy(_.name).zip(expected.fields.sortBy(_.name)).foreach { case (a, e) => {
  ...
  def rowRepr(row: Row, accuracy: Int): Seq[String] = {
    ...
    (0 until row.length).map(i => {
      ...
      else row.schema(i).dataType.sql match {
        case "DOUBLE" | "FLOAT" => d2s(row.get(i))
        case "ARRAY<DOUBLE>" => row.getAs[mutable.WrappedArray[Any]](i).map(x => d2s(x)).toList.mkString("Array(", ",", ")")
  ...
  def compareColumns(
                      ...
                      accuracy: Int = 7,
                      orderby: String = "uid"
                    )(implicit spark: SparkSession): Assertion =
  ...
  def compareDoubleColumns(
    ...
    def getSeq(df: DataFrame, name: String): Seq[String] =
      df.orderBy(orderby)
        .selectExpr(s"cast(${name} as double) as ${name}").as[Option[Double]]
        .collect
        .map(_.map(v => s"%1.${accuracy}f".format(v)).getOrElse("None")).toList
  ...
  def compareArrayDoubleColumns(
    ...
    def getSeq(df: DataFrame, name: String): Seq[String] =
      df.orderBy(orderby)
        .selectExpr(s"cast(${name} as array<double>) as ${name}").as[Option[Array[Option[Double]]]]
        .collect
        .map(optArr => optArr.map(
            arr => arr.map(
              optVal => optVal.map(
                v => s"%1.${accuracy}f".format(v)
              ).getOrElse("None")).mkString("Array(", ",", ")")
  ...
  def compareStringColumns(
    ...
    def getSeq(df: DataFrame, name: String): Seq[String] =
      df.orderBy(orderby)
        .selectExpr(s"cast(${name} as string) as ${name}").as[Option[String]]
        .collect

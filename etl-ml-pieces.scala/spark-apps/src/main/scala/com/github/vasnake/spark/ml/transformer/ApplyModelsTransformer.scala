...
/** APPLY stage: load batch of models from models descriptions and apply that batch
  ...
  */
class ApplyModelsTransformer(override val uid: String)
    extends Transformer
  ...
  /** transformer parameter: list of models encoded to string by custom encoder
    */
  final val models: Param[String] = new Param[String](this, "models", "List of models descriptions")
  setDefault(models, "")
  ...
  /** transformer parameter: df columns to keep
    */
  final val keep_columns: Param[String] =
    new Param[String](this, "keep_columns", "List of columns names to keep during transformation")
  ...
  protected def buildConfig(): ApplyModelsTransformerConfig = {
    ...
    logInfo(
      s"Columns to keep: <${getKeepColumns}>, decoded as [${keepColumns.mkString("'", "', '", "'")}]"
    )

    val modelsList: Seq[ComplexMLModel] = {
      logInfo(s"Decoding models description: <${getModels}> ...")
      val descriptions: Seq[ModelDescription] =
        DescriptionTools.extractModelsDescriptions(getModels)
  ...
  protected def loadModel(description: ModelDescription): ComplexMLModel = {
    ...
    modelType match {
      ...
      case Some("LalBinarizedMultinomialNbPLSA") =>
        unbox(loadLalBinarizedMultinomialNbModel(description))
      case Some("BaseLineDummy") => BaseLineModel(description) // n.b. not a best way to facilitate it tests
      case _ =>
        logError(
          s"model loading failed, unknown model type '${modelType}'; description '${description}'"
        )
...
object ApplyModelsTransformer extends DefaultParamsReadable[ApplyModelsTransformer] {
  ...
  private def applyModelsToRow(row: Row, cfg: ApplyModelsDistributedConfig): Seq[Row] = {
    ...
    cfg.models_featuresIdx.map {
      case (model, featuresIndices) =>
        Row.fromSeq(
  ...
  object DescriptionTools {
    ...
    val CFG_MODELS_SEPARATOR: String = ";" // list of models: model_descr1;model_descr2;...
    val CFG_MODEL_PARAMETERS_SEPARATOR: String = "," // list of pairs: k1->v1,k2->v2,...
    val CFG_MODEL_KV_SEPARATOR: String = "->" // pair: a->b
    val CFG_COLUMNS_SEPARATOR: String = "," // list of names: col1,col2,...

    val CFG_MODEL_TYPE_KEY: String = "model_type" // fixed model type name is stored under that key
    ...
    private def packModelDescription(descr: ModelDescription): String =
      descr
        .toList
        .map { case (k, v) => s"${k}${DescriptionTools.CFG_MODEL_KV_SEPARATOR}${v}" }
        .sorted
  ...
  def loadLalBinarizedMultinomialNbModel(description: ModelDescription)
    : Try[LalBinarizedMultinomialNbModel] = Try {
    val audienceName = description("audience_name")
  ...
  def loadLalTfidfScaledSgdcModel(description: ModelDescription): Try[LalTfidfScaledSgdcModel] =
    Try {
      val audienceName = description("audience_name")
      val equalizerSelector = description("equalizer_selector")
  ...
  private def readTextFile(path: String): String = {
    ...
    val tryLocalPath = trySparkFiles.recoverWith {
      case err =>
    ...
    val tryHdfs = tryLocalPath.recover {
      case err =>

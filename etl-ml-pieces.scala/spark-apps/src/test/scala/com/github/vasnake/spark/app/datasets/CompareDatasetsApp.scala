...
object CompareDatasetsApp {
  ...
  */
  ...
  object Functions {
    ...
    def loadHivePartition(db: String, table: String, dt: String, uid_type: String)(implicit spark: SparkSession): DataFrame = {
      ...
      spark.read.table(s"${db}.${table}")
        .where($"dt" === lit(dt))
        .where($"uid_type" === lit(uid_type))
        .drop("dt", "uid_type")
    }

    def comparePartitions(got: DataFrame, expected: DataFrame, ops: DataFrameActions): DataFrame = {
    ...
    def packColumnsToStruct(df: DataFrame, structAlias: String, keyColName: String = "uid"): DataFrame =
      df.select(
        df(keyColName),
        sqlfn.struct(
    ...
    def packAndJoin(
                     ...
                     leftName: String = "prod", rightName: String = "dev",
...
//      import spark.implicits._
      ...
      val withDiff = df.mapPartitions(rows => {
        ...
        def genericDiff(
                         left: Any, right: Any, dataType: sql.types.DataType,
                         leftName: String = "prod", rightName: String = "dev"
                       ): String = {
          import sql.types._
          import scala.collection.mutable
          ...
          def arraysDiff[T](left: mutable.WrappedArray[Any], right: mutable.WrappedArray[Any], cmp: (T, T) => Int): String = {
            if(left.length == right.length) {
              val diff: Seq[String] = (0 until left.length).foldLeft(Seq.empty[String])((acc, i) => {
                ...
                else acc :+ { if (left(i) == null) s"${leftName} is null idx ${i}" else s"${rightName} is null idx ${i}" }
              })
          ...
          def mapsDiff[T](left: Map[String, Any], right: Map[String, Any], cmp: (T, T) => Int): String = {
              ...
              val diff: Seq[String] = {
                left.keys.foldLeft(Seq.empty[String])((acc, key) => {
                  right.get(key) match {
                    ...
                    case Some(rv) => {
                      val lv = left.getOrElse(key, sys.error(s"No value in ${leftName}, impossible!"))
                      ...
                      if (lv != null && rv != null) {
                        ...
                        if (lv == null) s"${leftName} is null key ${key}" else s"${rightName} is null key ${key}"
                      }
                    }
                  }
                })
          ...
          dataType match {
            case ArrayType(DataTypes.IntegerType, _) => arraysDiff[Int](toArray(left), toArray(right), cmpInt)
            case ArrayType(DataTypes.FloatType, _) => arraysDiff[Float](toArray(left), toArray(right), cmpFloat)
            case MapType(DataTypes.StringType, DataTypes.IntegerType, _) => mapsDiff[Int](toMap(left), toMap(right), cmpInt)
            case MapType(DataTypes.StringType, DataTypes.FloatType, _) => mapsDiff[Float](toMap(left), toMap(right), cmpFloat)
...
//    import scala.collection.mutable
    ...
    val funcs = CopyPasteToSparkShell.Functions
    import CopyPasteToSparkShell._

    @transient
    implicit val spark: SparkSession = sql.SparkSession
      .builder()
      .config("hive.exec.dynamic.partition.mode", "nonstrict")
      .enableHiveSupport()
      .getOrCreate()

    val hdfsBasePath = "hdfs:/.../dev-prod-compare"
    ...
    def checkHidDataset30(_dt: String = "2023-01-08")(implicit spark: SparkSession): DataFrame = {
      ...
      funcs.checkpoint(df, s"${hdfsBasePath}/${prodTable}/diff")
        .persist(StorageLevel.MEMORY_AND_DISK_2)
    }

    val diff = checkHidDataset30("2023-03-02")
    diff.count()

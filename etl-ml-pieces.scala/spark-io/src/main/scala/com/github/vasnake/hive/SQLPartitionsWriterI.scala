...
STORED AS ORC
   ...
   */
  def createTableDDL(dbName: String, tableName: String, partitionColNames: Seq[String], schema: StructType): Try[String]
...
SELECT * FROM `mydb_mytable_source_temp_view`
   ...
   */
  def insertStaticPartDML(dbName: String, tableName: String, partition: Map[String, String], df: DataFrame): Try[String]

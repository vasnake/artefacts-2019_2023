...
class SQLHiveWriter(spark: => SparkSession) extends SQLPartitionsWriterI with Logging {
  def createTableDDL(
    ...
    val columns: String = {
      val cols: Seq[StructField] =
        schema.fields.filter(field => !partitionColumnsNames.contains(field.name))
  ...
  def insertStaticPartDML(
    ...
    logDebug(s"Hive $dbName.$tableName part. cols: [${hiveParts
  ...
  def insertStaticPartDML(
    ...
    logInfo(
      s"insertStaticPartDML, db: `${dbName}`, table: `$tableName`, partition: [${partition.mkString(", ")}]"
    )
    logInfo(
      s"from DataFrame ${df.rdd.getNumPartitions} partitions; schema: ${df.schema.simpleString}"
    )
    logInfo(s"into Hive table `$dbName.$tableName`; part. cols: [${hiveParts
        .mkString(",")}]; non-part cols: [${hiveCols.mkString(",")}]")
    ...
    val tempViewName =
      quoteIdentifier(s"${dbName}__${tableName}__${slugify(partition)}__source_temp_view")
    ...
    val partitionExpr = hiveParts
      .map(colName => s"${quoteIdentifier(colName)}='${partition(colName)}'" // possible NoSuchElementException
      )

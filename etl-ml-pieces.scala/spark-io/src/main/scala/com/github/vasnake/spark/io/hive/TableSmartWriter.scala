...
import org.apache.spark.sql.catalyst.catalog.{CatalogTablePartition, ExternalCatalog}
import org.apache.spark.sql.catalyst.expressions.{Expression => CatalystExpression}
import org.apache.spark.sql.types.DataTypes
import org.apache.spark.sql.{Column, DataFrame, Row, SaveMode, SparkSession, functions => sqlfn}
...
class TableSmartWriter(partitionMethodOrNull: String) extends CustomLogging {
  ...
  def insertIntoHive(
    ...
    logInfo(s"Inserting DataFrame `${df.schema.simpleString}`\ninto Hive table `${database}.${table}`\nwith parameters: " +
    ...
    // part. columns in DF
    require(
      partColumnNames.forall(df.schema(_).dataType.simpleString == DataTypes.StringType.simpleString), // ignore nullability
      s"Dataframe partition columns must by of type `string`, got ${df.schema.simpleString}"
    )

    if (partColumnNames.isEmpty) logWarning(s"Table `${database}.${table}` doesn't have partitioning columns")
    else logInfo(s"Partition columns: (${partColumnNames.mkString(", ")})")

    val result: Try[Unit] = for {
      fixedDF <- fixDataFrameStructure(df, hiveColumns.map(_.name), recoverMissingCols = !raiseOnMissingColumns)
      (partDF, partStats, bucketingErrorsCount) <- repartitionToBuckets(fixedDF, partColumnNames, maxRowsPerBucket, partitionMethod)
      // remove success flag if present
      _ <- alterPartitionsParameters(database, table, partStats, checkParameter.map(name => Map(name -> None)).getOrElse(Map.empty), failIfNotFoundPartitions = false)
      // actual insert
      _ <- write(partDF, database, table, overwrite)
      _ <- if (bucketingErrorsCount.value > 0) Failure(new RuntimeException(s"Bucketing errors count: ${bucketingErrorsCount.value}")) else Success(Unit)
      // set success flag if present
      _ <- alterPartitionsParameters(database, table, partStats, checkParameter.map(name => Map(name -> Some("true"))).getOrElse(Map.empty))
    } yield ()

    // error logging and escalating
    result match {
      case Failure(exception) => {
        logError(s"Failed inserting dataframe into Hive table `${database}.${table}`: ${exception.getMessage}", exception)
        throw exception
      }
...
object TableSmartWriter {
  ...
  def fixDataFrameStructure(
                             df: DataFrame, hiveColumns: Seq[String], recoverMissingCols: Boolean
                           )(implicit log: Logger): Try[DataFrame] = Try {
  ...
  def repartitionToBuckets(
                            df: DataFrame, partitionColumnsNames: Seq[String], maxRowsPerBucket: Int, partitionMethod: String
                          )(implicit log: Logger): Try[(DataFrame, Option[DataFrameBucketsStats], LongAccumulator)] = Try {
    ...
    if (partitionColumnsNames.isEmpty) {
      ...
      if (rowsCount > 0) {
        ...
        log.info(s"Flat table repartition: rows: ${rowsCount}, rows-per-bucket: ${maxRowsPerBucket}, partitions: ${numParts}")
        // result
      ...
      for {
        ...
        bucketedDF <- partitionMethod match {
          case RDD_PARTITIONER => _repartitionToBucketsRdd(df, partitionsStats, maxRowsPerBucket, errorsCountAccum)
          case _ => _repartitionToBucketsSql(df, partitionsStats, maxRowsPerBucket, errorsCountAccum)
        }
  ...
  def _collectPartitionsStats(
                               df: DataFrame, partitionColumns: Seq[String], maxRowsPerBucket: Int
                             )(implicit log: Logger): Try[Option[DataFrameBucketsStats]] =
    Try {
      ...
      if (stats.dataFramePartitions.isEmpty) {
        ...
        log.debug(s"partitions (first 20 from ${stats.dataFramePartitions.length}):\n`${stats.dataFramePartitionsStats.take(20).mkString(";\n")}`")
        Some(stats)
      }
    }
  ...
  def _repartitionToBucketsRdd(
    ...
    val partitionedDF: DataFrame = partitionsInfo match {
      ...
      case Some(partsInfo) => {
        ...
        val bcPartsColsIndices: Broadcast[Seq[Int]] = sc.broadcast(partsInfo.partitionColumnsIndices)

        // df DAG ends here
        if (DEBUG_MODE) df.explain(extended = true)

        // partition id -> data row
        val kvRdd: RDD[(String, Row)] = df.rdd.keyBy(row =>
          bcPartsColsIndices.value.map(i => {
        ...
        val bcParams: Broadcast[(Random, Int, Map[String, (Int, Int)])] = sc.broadcast((
          ...
          partsInfo.dataFramePartitionsStats.map { case (k, v) => (k -> (v.firstBucket, v.bucketsCount)) }
        ))

        val bucketedKvRdd = kvRdd.partitionBy(new BucketPartitioner(bcParams, errorsCountAccum))

        log.debug("Creating output DF from RDD ...")
        val res = df.sparkSession.createDataFrame(
          bucketedKvRdd.mapPartitions(rows => rows.map(row => row._2), preservesPartitioning = true),
          df.schema
        )
  ...
  def _repartitionToBucketsSql(
    ...
    val partitionedDF: DataFrame = partitionsInfo match {
      ...
      case Some(partsInfo) => {
        ...
        val bucketNumColName: String = s"""${"a" * df.schema.fieldNames.map(_.length).max}_bucketN"""
        val totalBucketsCount: Int = partsInfo.dataFramePartitionsStats.values.map(_.bucketsCount).sum
        ...
        val res = dfWithBucketNum.repartitionByRange(totalBucketsCount * 2, sqlfn.col(bucketNumColName))
  ...
  def _createBucketingUDF(partitionsInfo: DataFrameBucketsStats, errorsCountAccum: LongAccumulator, spark: SparkSession): UserDefinedFunction = {
    ...
    val bcParams = spark.sparkContext.broadcast((
      new Random(),
      partitionsInfo.dataFramePartitionsStats.map { case (k, v) => (k -> (v.bucketsCount, v.firstBucket)) },
      partitionsInfo.dataFramePartitionsStats.values.map(_.bucketsCount).sum
    ))

    val getPartBucketNumber: String => Long = (partId: String) => {
      val (numBuckets, firstBucket) = bcParams.value._2.getOrElse(
        partId,
        {
  ...
  def alterPartitionsParameters(
    ...
    log.info(s"Updating Hive table partitions parameters `${params.mkString(", ")}` for ${partStats.map(_.dataFramePartitions.length).getOrElse(0)} partitions ...")
    ...
    partStats match {
      ...
      case Some(stats) => {
        if (params.isEmpty) log.warn("Have no parameters to set")
          ...
          val mqpOpt: Option[MetastoreQueryProcessorWithConnPool] = MetastoreQueryProcessorWithConnPool(spark, poolSize)
          log.debug(s"Connections pool size, actual: ${mqpOpt.map(_.poolActualSize)}")

          log.info(s"Filtering metastore partitions ...")
          val partitions: Seq[CatalogTablePartition] = _listPartitions(database, table, stats.dataFramePartitions, mqpOpt)
          ...
          if (partitions.nonEmpty) {
            log.debug(s"Filtered metastore partitions: first 20:\n${partitions.take(20).mkString(";\n")}")
            val counts = partitions.map(p => p.parameters.getOrElse("numFiles", "0").toInt)
            log.debug(s"Number of files in partitions (by catalog), list (first 99): [${counts.take(99).mkString(",")}]")
            log.info(s"Number of files in partitions (by catalog), total: ${counts.sum}")

            val updatedParts = partitions.map(p => p.copy(parameters = _updateData(data = p.parameters, updates = params)))
  ...
  def _listPartitions(
    ...
    _processPartitions(filters, askCatalogFun, mqp)
      .toArray
  }
  ...
  def _alterPartitions(
    ...
    val chunks: Array[Seq[CatalogTablePartition]] =
      _splitToChunks(parts, PARTITIONS_LIST_WRITE_CHUNK_SIZE)
        .toArray
  ...
  private def _splitToChunks[T](xs: Seq[T], chunkSize: Int): Seq[Seq[T]] = xs.sliding(chunkSize, chunkSize).toSeq
  ...
  def write(df: DataFrame, database: String, table: String, overwrite: Boolean)(implicit log: Logger): Try[Unit] = {
  ...
  class BucketPartitioner(
    ...
    override def getPartition(key: Any): Int = key match {
      ...
      case partitionID: String => {
        bucketsInfo.get(partitionID).map { case (firstBucket, bucketsCount) => firstBucket + rnd.nextInt(bucketsCount) }
          .getOrElse({
  ...
  val PARTITION_OVERWRITE_MODE_KEY = "spark.sql.sources.partitionOverwriteMode"
  val TIME_ZONE_ID = "UTC" // timezone id to parse partition values of TimestampType, for ExternalCatalog

  val RESERVED_PART_PARAM_NAMES = Set(
    "rawDataSize",
    "numFiles",
  ...
  val SQL_PARTITIONER = "SQL"
  val RDD_PARTITIONER = "RDD"
  ...
  val METASTORE_CONNECTIONS_POOL_SIZE_KEY = "spark.hadoop.vasnake.metastore.pool.size"
  val METASTORE_CONNECTIONS_POOL_SIZE_DEFAULT = 8
  ...
  def computeConnectionsPoolSize(spark: SparkSession, partitionsTotalCount: Int): Int = {
    ...
    val param = math.max(
      1,
      _poolSizeParameterValue(spark, METASTORE_CONNECTIONS_POOL_SIZE_KEY, METASTORE_CONNECTIONS_POOL_SIZE_DEFAULT)
    )
  ...
  private def _poolSizeParameterValue(spark: SparkSession, key: String, default: Int) = Try(
    spark.conf.get(key, default.toString).toInt
  ).getOrElse(default)

  private def _poolSize(maxSize: Int, partitionsTotalCount: Int, chunkSize: Int) = {
  ...
  def debugShow(df: => DataFrame, message: => String = "", nrows: Int = 200, truncate: Boolean = false, force: Boolean = false): Unit =
    if (DEBUG_MODE || force) {

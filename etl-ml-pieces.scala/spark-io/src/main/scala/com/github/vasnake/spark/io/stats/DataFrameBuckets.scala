...
case class DataFrameBucketsStats(
  ...
  def setupBuckets(maxRowsPerBucket: Int): DataFrameBucketsStats = {
    ...
    val stats = dataFramePartitionsStats.map {
      case (partKey, counts) =>
...
object DataFrameBucketsStats {
  def apply(
    ...
    val countsDF: DataFrame = df
      .groupBy(partitionColumns.map(name => sqlfn.col(name)): _*)
      .agg(sqlfn.count(sqlfn.lit(1)).cast(DataTypes.StringType) alias countColumnName) // unify result data types to string

    val partitionsWithRowsCount: Seq[Map[String, String]] = countsDF
      .collect()
      .map(row => row.getValuesMap[String](partitionColumns :+ countColumnName) // N.B. row.getValuesMap[String] doesn't really do conversion to String
      )
    ...
    if (partitionsWithRowsCount.isEmpty) new DataFrameBucketsStats() // empty stats from empty dataframe
      ...
      new DataFrameBucketsStats(
        ...
        dataFramePartitions =
          partitionsWithRowsCount.map(row => row.filterKeys(_ != countColumnName).toHashMap),

        // map part_key: counts for each partition
        dataFramePartitionsStats = partitionsWithRowsCount
          .map(row =>
            buildPartitionID(row, partitionColumns) -> DataFrameBuckets(rowsCount =
              row(countColumnName).toLong
            )
          )
          .toMap
          .toHashMap,
  ...
  def summaryMessage(stats: DataFrameBucketsStats): String = {
    ...
    val keysCountStr = {
      val counts: Map[String, Int] = stats
        .dataFramePartitions
        .flatMap(_.toSeq) // Seq[(String, String)]

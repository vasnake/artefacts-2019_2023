...
class MetastoreQueryProcessorWithConnPool(
  ...
  require(!isDerby(spark), "org.apache.derby can't be used in connections pool, set pool size to 1 for using vanilla externalCatalog")
  require(isHive(spark), "spark.sql.catalogImplementation must be 'hive' for using connections pool, set pool size to 1 for using vanilla externalCatalog")
  ...
  def closeConnections(): Unit = {
    ...
    connections.zipWithIndex.foreach {
      case (conn, idx) => Try {
        ...
        case Failure(exception) => log.warn(s"Failed closing connection ${idx + 1}: ${exception.getMessage}", exception)
        case _ => log.debug(s"Connection ${idx + 1} successfully closed.")
      }
    }
  }

  def processQueries[A, B](queries: IndexedSeq[A], oneQueryFun: (A, ExternalCatalog) => Seq[B]): IndexedSeq[B] = {
    ...
    log.debug(s"Planning to execute total ${queries.length} queries, on ${poolActualSize} connections, with ${numQueriesPerConn} queries per connection, with ${reminder} queries in the last round")

    val res = {
      if (numQueriesPerConn == 0) {
        // num of queries is less than num of connections
        connections.take(reminder).zipWithIndex
          .par // spawn x threads, x = remainder
          .flatMap { case (catalog, idx) => {
            withSafeClose(
        ...
        connections.zipWithIndex
          .par // spawn x threads, x = poolActualSize
          .flatMap { case (catalog, catIdx) => {
            withSafeClose(
              // sequence of queries on one connection
              fun = sliceIndices(numQueriesPerConn, reminder, catIdx, queries.length).flatMap(idx => _runQuery(idx, catalog)),
              close = HiveExternalCatalog.closeConnection(catalog)
            )
          }}
      }
  ...
  private def withSafeClose[T](fun: => T, close: => Unit): T = {
    ...
    Try(close) match {
      case Failure(exception) => log.warn(s"Failed closing connection: ${exception.getMessage}", exception)
...
object  MetastoreQueryProcessorWithConnPool extends CustomLogging {
  ...
  val METASTORE_CONNECTIONS_POOL_SIZE_MAX = 32
  ...
  def preparePool(poolSize: Int, spark: SparkSession): Unit = {
    require(poolSize <= METASTORE_CONNECTIONS_POOL_SIZE_MAX, s"Pool max size cannot be greater than ${METASTORE_CONNECTIONS_POOL_SIZE_MAX}")
    ...
    if (connections.length >= poolSize) {
      ...
      connections = connections ++ (1 to newItemsCount).map(_ => HiveExternalCatalog.openConnection(spark))
  ...
  def sliceIndices(numItems: Int, reminder: Int, sliceIdx: Int, totalItems: Int): Seq[Int] = {
    assert(

...
object TopNRowsExact {
  ...
  val globalRankRDD: GlobalRank = new GlobalRank {
    ...
    override def add(
      ...
      // decode from RDD
      val origColumns = df.schema.fieldNames.map { name =>
        sf.col("_1").getItem(name).alias(name)
      }
  ...
  val globalRankSQL: GlobalRank = new GlobalRank {
    ...
    override def add(
      ...
      val withLocalRankDF =
        withPartitionIdDF.withColumn(localRankColumn, sf.rank().over(partitionW)) // rank or row_number?
      ...
      val statsDF = {
        ...
        val maxLocalRankDF = withLocalRankDF
          .groupBy(partIdColumn)
          .agg(sf.max(localRankColumn).alias(maxLocalRankColumn))

        // DF has rows.count = partitions.count, no need to do `partitionBy` in window definition
        val prevRowsW =
          Window.orderBy(partIdColumn).rowsBetween(Window.unboundedPreceding, Window.currentRow)

        // rows count in partition and all previous partitions
        val cachedStatsDF = maxLocalRankDF
          .withColumn(cumSumMaxRankColumn, sf.sum(maxLocalRankColumn).over(prevRowsW))
          .cache() // TODO: check if it helps, probably not
        debug(
          s"GlobalRank, force cache materialization, stats DF rows count: ${cachedStatsDF.count()}"
        )
        cachedStatsDF
      }
      ...
      val withTotalRankDF = withLocalRankDF
        .join(sf.broadcast(shiftedStatsDF), Seq(partIdColumn), "inner")

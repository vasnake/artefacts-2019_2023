...
class PrimitiveIntTest extends AnyFlatSpec with should.Matchers with LocalSpark with Checks {
  ...
  lazy val inputDF: DataFrame = cache(
    createInputDF(spark)
      ...
      .selectExpr("part", "uid", "cast(feature as integer) as feature")
      .repartition(4)
  )

  val resultColumnType: String = "IntegerType"
  ...
  it should "ignore invalid if valid values exists, part D" in {
    ...
    minAndCheck(input, "0")
    maxAndCheck(input, "4")
    avgAndCheck(input, ((4 + 3) / 4).toString)
    mfqAndCheck(input, "0")
  }

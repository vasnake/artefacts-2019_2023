...
class ScoreQuantileThresholdTest
    extends AnyFlatSpec
  ...
  it should "fail if output column exists already, part G" in {
    ...
    val input = inputDF
      .where("part = 'G'")
      .selectExpr("uid", "score", "expected_index as c", "expected_rank as r")
  ...
  it should "transform invalid score to null, part A" in {
    ...
    val input = inputDF
      .selectExpr("uid", "score_train", "score", "expected_index", "expected_rank")
      .where("part = 'A'")

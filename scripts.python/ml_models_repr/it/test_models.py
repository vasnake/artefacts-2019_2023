...
class TestLearnApply(object):
    ...
    def test_models_equal(self, session_temp_dir, clf_learnset_factory):
        from prj.adapters.model.oclf import (
            OrdClfSGDRegressor, OrdClfLGBMRegressor,
            OrdClfSGDRegressor as OrdClfSGDRegressor_2,
            OrdClfLGBMRegressor as OrdClfLGBMRegressor_2
        )
        from prj.adapters.model.lal import (
            LookalikeMultinomialNB, LookalikeBernoulliNB, LookalikeLGBMClassifier,
            LookalikeMultinomialNB as LookalikeMultinomialNB_2,
            LookalikeBernoulliNB as LookalikeBernoulliNB_2,
            LookalikeLGBMClassifier as LookalikeLGBMClassifier_2
        )
        from prj.adapters.model.clf import (
            ClfMultinomialNB, ClfBernoulliNB, ClfLGBMClassifier,
            ClfMultinomialNB as ClfMultinomialNB_2,
            ClfBernoulliNB as ClfBernoulliNB_2,
            ClfLGBMClassifier as ClfLGBMClassifier_2,
        )
        ...
        assert testset.x.shape == (20, 20)

        models1 = [
            OrdClfSGDRegressor(),
            OrdClfLGBMRegressor(),
            LookalikeMultinomialNB(),
            LookalikeBernoulliNB(),
            # LookalikeSGDClassifier(),
            LookalikeLGBMClassifier(),
            ClfMultinomialNB(),
            ClfBernoulliNB(),
            # ClfSGDClassifier(),
            ClfLGBMClassifier()
        ...
        models2 = [
            OrdClfSGDRegressor_2(),
            OrdClfLGBMRegressor_2(),
            LookalikeMultinomialNB_2(),
            LookalikeBernoulliNB_2(),
            # LookalikeSGDClassifier_2(),
            LookalikeLGBMClassifier_2(),
            ClfMultinomialNB_2(),
            ClfBernoulliNB_2(),
            # ClfSGDClassifier_2(),
            ClfLGBMClassifier_2()
        ...
        for model1, model2 in zip(models1, models2):
            print("\nlearn {} ...".format(type(model1)))
            model1 = model1.learn(learnset)
            print("\nlearn {} ...".format(type(model2)))
            model2 = model2.learn(learnset)
            print("\napply {} ...".format(type(model1)))
            rows1 = list(model1.apply(testset))
            print("\napply {} ...".format(type(model2)))
            rows2 = list(model2.apply(testset))
            # results["{}".format(type(model1))] = rows1
            assert len(rows1) == test_size and len(rows2) == test_size
        ...
        # with open("/tmp/reference-predicts.json", "w") as f:
        #     json.dump(results, f, sort_keys=True, indent=4)

    def test_apply_speed_clf(self, session_temp_dir, clf_learnset_factory):
        from prj.adapters.model.oclf import (
            OrdClfSGDRegressor as OrdClfSGDRegressor_2
        )
    ...
    def test_compare_models(self, session_temp_dir, make_lal_learning_data):
